...
import { Droplets, ArrowRight, ArrowLeft, CheckCircle2 } from "lucide-react";
import Header from "../../../components/Header";
import Footer from "../../../components/Footer";
import EnquiryModal from "../../../components/EnquiryModal";
import { getEcoProductsByCategory } from "../../../data/products";

export default function EcoLiquidCleanersPage() {
  const products = getEcoProductsByCategory("liquid-cleaners");
    ...
    <div className="min-h-screen flex flex-col font-sans bg-gradient-to-br from-[#FAF6F0] via-[#F5EFE6] to-[#EFEBE4] text-[#2C3531]">
      ...
      <section className="bg-gradient-to-r from-[#042834] via-[#084C61] to-[#1B5E20] text-white py-12 sm:py-16 shadow-lg">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-4 animate-slide-left">
          ...
          <h1 className="font-serif text-3xl sm:text-5xl font-bold flex items-center gap-3">
            <Droplets className="w-8 h-8 text-[#FFD1B3]" /> Eco Liquid Cleaners & Refills
          </h1>
          <p className="text-sm sm:text-base text-slate-200 max-w-3xl font-normal">
            Real lemon extract dishwash gels, plant-derived machine washing liquids, organic neem floor disinfectants, and zero-waste wholesale refill drums.
          </p>
        </div>
      </section>
      ...
      <section className="py-8 bg-white/60 backdrop-blur-md border-b border-[#E8DFD5] text-xs sm:text-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-2 md:grid-cols-4 gap-4 text-center font-bold text-[#2E7D32]">
          <div className="eco-glass-panel p-3.5 rounded-2xl border border-white/80">Cold-Pressed Citrus Limon Oil</div>
          <div className="eco-glass-panel p-3.5 rounded-2xl border border-white/80">Zero Synthetic Parabens</div>
          <div className="eco-glass-panel p-3.5 rounded-2xl border border-white/80">Safe For Greywater Irrigation</div>
          <div className="eco-glass-panel p-3.5 rounded-2xl border border-white/80">Returnable 50kg Refill Drums</div>
        </div>
      </section>
